...
const car = {
    ...
    refill: function(amount){
        if (this.petrolLevel + amount > this.petrolCapacity) {
            let filledPetrol = this.petrolCapacity - this.petrolLevel
            console.log('Car filled with ' + filledPetrol + ' liter of petrol.')
            this.petrolLevel += this.petrolCapacity - this.petrolLevel;
            return filledPetrol
        } else {
            let currentPetrolLevel = this.petrolLevel;
            this.petrolLevel += amount;
            let filledPetrol = this.petrolLevel - currentPetrolLevel;
            console.log('Car filled with ' + filledPetrol + ' liter of petrol.')
            return filledPetrol
        }
    }
